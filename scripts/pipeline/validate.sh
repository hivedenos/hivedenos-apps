...
jq -e '.apps | all(.[]; has("id") and has("name") and has("version") and has("description") and has("repository_path") and has("source") and has("install") and has("search") and has("updated_at"))' "$catalog_file" >/dev/null
